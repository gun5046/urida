...
def generate_character_labels(transcripts, labels_dest):
    ...
    label_df.to_csv(os.path.join(labels_dest, "jong_character_vocabs.csv"), encoding="utf-8", index=False)


def generate_character_script(audio_paths, transcripts, labels_dest):
    print('create_script started..')
    char2id, id2char = load_label(os.path.join(labels_dest, "jong_character_vocabs.csv"))
